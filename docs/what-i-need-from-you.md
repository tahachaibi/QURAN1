...
## 2b. ~~The adhan~~ — done, and verified this time

**Abd Elmajid Essebihi, 3:33, bundled.** It arrived as a 37.5 MB stereo PCM WAV
inside a zip, and 37.5 MB is not a thing to put in an APK, so it was encoded here
to mono 44.1 kHz MP3 at 96 kbps — **2.44 MB**. Mono because an adhan is one voice;
the sample rate left alone because resampling 44.1 kHz down needs a proper filter
and a cheap one would put aliasing into a recitation.

**Checked for a signal before being committed**, which is the step that was missed
last time: peak −2.1 dBFS, RMS −18.6 dBFS, 95.7% non-zero samples, and a
per-second energy profile with the shape of a muezzin — phrases separated by
breaths:
...
The encoded file then passed `scripts/verify-audio.mjs`: 9.83% zero bytes, all 256
byte values in use. The file before it: **100% zero bytes, one byte value.**

### The two things that now stop this recurring

- `verify-audio.mjs` runs in `npm run gen` and in CI: it walks the MPEG frames,
  measures the zero-byte fraction of the audio payload, and **fails above 95%**.
- **Choose adhan** on the prayer tab still lets you pick any file on the phone,
  at any size, if you would rather use a different recitation. A chosen file wins
  over the bundled one.

### Install note

The bundled recording is also the notification sound for when the app is closed.
Android freezes a notification channel's sound when the channel is created, so
**uninstall the old app before installing this build** or the prayer-time
notification will stay silent. In-app playback — the part with the Stop button —
works either way.
